...
get_karoo_data <- function() {
    ...
    species <- read.delim("~/MATSS/inst/extdata/sanparks/peggym.114.1-KarooNationalParkCensuscodes.txt") %>%
    ...
    karoo1 = read.delim("~/MATSS/inst/extdata/sanparks/peggym.116.1-KRNPTotals.txt") %>%
    ...
    karoo2 = read.delim("~/MATSS/inst/extdata/sanparks/peggym.1050.1-Karoo2006.txt") %>%
        dplyr::mutate(Date = as.Date(Date)) %>%
    ...
    karoo3 = read.delim("~/MATSS/inst/extdata/sanparks/peggym.1049.1-karoo2008.txt") %>%
        dplyr::mutate(Date = as.Date(Date)) %>%
    ...
    karoo4 = read.delim("~/MATSS/inst/extdata/sanparks/peggym.1051.1-Karoo09.txt") %>%
        dplyr::mutate(Date = as.Date(Date)) %>%
        dplyr::rename(Common.name = "CONTROL", total = "TOTAL") %>%
        dplyr::mutate(Common.name = gsub("\\s+","",Common.name)) %>%
        dplyr::mutate(Common.name = replace(as.character(Common.name), 
    ...
    karoo <- list(abundance = abundance, covariates = covariates, metadata = metadata)
...
kruger_raw <- read.delim("~/MATSS/inst/extdata/sanparks/judithk.815.1-815.1.txt") %>%
...
kruger <- list(abundance = abundance, covariates = covariates, metadata = metadata)
